...
# PS
# ---
       my @AllParamNames = $ParamObject->GetParamNames();
       my @NewParamNames = grep{ $_ =~ m{\ANewParam (?:Key|Value) [0-9]+ \z}xms }@AllParamNames;
# ---
...
# PS
# ---
    my $MaxNumber = $ConfigObject->Get('GenericAgent::MaxNewParam') || 6;
    for my $Number ( 1 .. $MaxNumber ) {
        $LayoutObject->Block(
            Name => 'NewParam',
            Data => {
                ParamNumber => $Number,
                KeyValue    => $JobData{ "NewParamKey" . $Number },
                ParamValue  => $JobData{ "NewParamValue" . $Number },
            },
        );
    }
# ---
